...
const renderElement = (element: PolicyElement, order: Order) => {
    ...
    switch (element.type) {
        case 'text':
            return (
                <div style={{ ...style, color: element.color, fontSize: `${element.fontSize}px`, fontWeight: element.fontWeight, whiteSpace: 'nowrap', padding: '2px' }}>
        ...
        case 'image':
             if (element.content) {
                return <img src={element.content} alt="Policy element" style={{...style, objectFit: 'contain'}} />;
            }
...
export const PrintablePolicy = forwardRef(({ orders, template }: PrintablePolicyProps, ref) => {
    ...
    return (
        <div className="w-full">
            ...
            {/* This button is primarily for the sample print from the editor */}
            {orders.length === 0 && (
