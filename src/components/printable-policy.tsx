...
const BarcodeIcon = () => (
  <svg viewBox="0 0 120 30" className="h-10 w-24">
    <rect x="0" y="0" width="2" height="30" fill="black" /> <rect x="4" y="0" width="1" height="30" fill="black" /> <rect x="7" y="0" width="3" height="30" fill="black" /> <rect x="12" y="0" width="1" height="30" fill="black" /> <rect x="15" y="0" width="2" height="30" fill="black" /> <rect x="19" y="0" width="2" height="30" fill="black" /> <rect x="23" y="0" width="1" height="30" fill="black" /> <rect x="26" y="0" width="3" height="30" fill="black" /> <rect x="31" y="0" width="1" height="30" fill="black" /> <rect x="34" y="0" width="3" height="30" fill="black" /> <rect x="39" y="0" width="2" height="30" fill="black" /> <rect x="43" y="0" width="1" height="30" fill="black" /> <rect x="46" y="0" width="1" height="30" fill="black" /> <rect x="49" y="0" width="3" height="30" fill="black" /> <rect x="54" y="0" width="2" height="30" fill="black" /> <rect x="58" y="0" width="1" height="30" fill="black" /> <rect x="61" y="0" width="3" height="30" fill="black" /> <rect x="66" y="0" width="1" height="30" fill="black" /> <rect x="69" y="0" width="2" height="30" fill="black" /> <rect x="73" y="0" width="2" height="30" fill="black" /> <rect x="77" y="0" width="1" height="30" fill="black" /> <rect x="80" y="0" width="3" height="30" fill="black" /> <rect x="85" y="0" width="1" height="30" fill="black" /> <rect x="88" y="0" width="3" height="30" fill="black" /> <rect x="93" y="0" width="2" height="30" fill="black" /> <rect x="97" y="0" width="1" height="30" fill="black" /> <rect x="100" y="0" width="1" height="30" fill="black" /> <rect x="103" y="0" width="3" height="30" fill="black" /> <rect x="108" y="0" width="2" height="30" fill="black" /> <rect x="112" y="0" width="1" height="30" fill="black" /> <rect x="115" y="0" width="3" height="30" fill="black" />
  </svg>
);
...
const renderCustomFields = (customFields: {label: string, value: string}[], isSmallLabel: boolean, order: Order) => (
...
const DefaultLayout = ({ settings, order, isSmallLabel }: { settings: any, order: Order, isSmallLabel: boolean }) => (
    <>
        ...
        <main className="flex-grow my-4 grid grid-cols-2 gap-4">
            <div className="border border-black rounded p-2">
                ...
                <p>{order.merchant}</p>
                <p>{/* Merchant Phone - Assuming it's available in a future version */}</p>
            </div>
            <div className="border border-black rounded p-2">
        ...
        {settings.showItems && (
        <div className="mb-4">
            {/* Items table would go here if item data was available on the order */}
        </div>
        )}
        {renderCustomFields(settings.customFields, isSmallLabel, order)}

        <footer className="mt-auto pt-2 space-y-4">
            <div className="grid grid-cols-2 gap-4">
                <div>
                    {settings.showRefNumber && <p className="text-xs">الرقم المرجعي: <span className="font-bold">{order.referenceNumber}</span></p>}
...
const CompactLayout = ({ settings, order, isSmallLabel }: { settings: any, order: Order, isSmallLabel: boolean }) => (
    <>
        <div className={cn("grid grid-cols-2 gap-x-2 border-b-2 border-black", isSmallLabel ? 'pb-1' : 'pb-2')}>
            <div className="text-right space-y-1">
                ...
                {settings.showCompanyName && <h1 className={cn("font-bold", isSmallLabel ? "text-[8px]" : "text-base")}>شركة الوميض</h1>}
            </div>
        ...
        <main className={cn("flex-grow space-y-1", isSmallLabel ? 'my-1' : 'my-2')}>
            <div className={cn("grid grid-cols-2 gap-1", isSmallLabel ? 'text-[7px]' : 'text-xs')}><div className='space-y-0.5'><p className='font-bold'>المرسل:</p><p>{order.merchant}</p></div><div className='space-y-0.5'><p className='font-bold'>الرقم المرجعي:</p><p>{order.referenceNumber}</p></div><div className='space-y-0.5'><p className='font-bold'>تاريخ الطلب:</p><p>{new Date(order.date).toLocaleDateString('ar-JO')}</p></div></div>
        ...
        {settings.showItems && (<div className={cn(isSmallLabel ? 'text-[7px] my-1' : 'text-[10px] my-2')}></div>)}
        {renderCustomFields(settings.customFields, isSmallLabel, order)}
...
const DetailedLayout = ({ settings, order, isSmallLabel }: { settings: any, order: Order, isSmallLabel: boolean }) => (
    <>
        ...
        <main className={cn("flex-grow space-y-1", isSmallLabel ? 'my-1' : 'my-2', isSmallLabel ? 'text-[7px]' : 'text-xs')}>
            <div className="grid grid-cols-2 gap-2"><p><span className="font-bold">تاريخ الطلب:</span> {new Date(order.date).toLocaleDateString('ar-JO')}</p>{settings.showRefNumber && <p><span className="font-bold">الرقم المرجعي:</span> {order.referenceNumber}</p>}</div>
            <div className="grid grid-cols-2 gap-2">
                <div className="border border-black rounded p-1"><h3 className="font-bold border-b border-black mb-1">المرسل</h3><p>{order.merchant}</p></div>
...
const defaultSettings = {
    paperSize: 'a4', layout: 'default', showCompanyLogo: true, showCompanyName: true,
    showCompanyAddress: false, showRefNumber: true, showItems: false,
    showPrice: true, showBarcode: true, footerNotes: 'شكرًا لثقتكم بنا.',
    customFields: []
};

export const PrintablePolicy = ({ orders }: { orders: Order[] }) => {
    ...
    const renderLayout = (order: Order) => {
    ...
    if (!orders || orders.length === 0) {
        return null; // Or a message indicating no orders to print
    }
        ...
        <div className="font-sans text-black bg-white">
            {orders.map((order, index) => (
                <div
                    key={order.id}
                    className={cn(
                        "bg-white shadow-lg mx-auto font-sans flex-shrink-0 flex flex-col",
                        paperSizeClasses[settings.paperSize as keyof typeof paperSizeClasses],
                        index < orders.length - 1 && "break-after-page" // Add page break for all but the last order
                    )}
                >
                    {renderLayout(order)}
                </div>
