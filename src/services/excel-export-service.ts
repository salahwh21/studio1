...
async function generateSlipWorksheet(workbook: ExcelJS.Workbook, slip: DriverSlip | MerchantSlip, users: User[], reportsLogo: string | null, isDriver: boolean) {
    ...
    // Add logo
    if (reportsLogo?.startsWith('data:image')) {
    ...
    if (barcodeBase64?.startsWith('data:image')) {
    ...
    const user = isDriver ? users.find(u => u.name === partyName) : users.find(u => u.storeName === partyName);
...
export async function generateExcel(slips: (DriverSlip | MerchantSlip)[], users: User[], reportsLogo: string | null, isDriver: boolean, filename: string) {
...
export const generateDriverSlipExcel = (slips: DriverSlip[], users: User[], reportsLogo: string | null) =>
    generateExcel(slips, users, reportsLogo, true, `driver_slips_${new Date().toISOString().slice(0,10)}.xlsx`);

export const generateMerchantSlipExcel = (slips: MerchantSlip[], users: User[], reportsLogo: string | null) =>
    generateExcel(slips, users, reportsLogo, false, `merchant_slips_${new Date().toISOString().slice(0,10)}.xlsx`);
